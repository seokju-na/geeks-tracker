...
pub fn get_current_branch_name(repo: &Repository) -> Result<String, GitError> {
    ...
    parse_branch_name(reference.name().unwrap_or(""))
}

pub fn switch_branch(repo: &Repository, branch_name: &str) -> Result<(), GitError> {
    // do nothing if already in same branch.
    if let Ok(current_branch_name) = get_current_branch_name(&repo) {
...
fn branch_name_to_ref_name(branch_name: &str) -> String {
    format!("refs/heads/{}", branch_name)
}

fn parse_branch_name(ref_name: &str) -> Result<String, GitError> {
...
mod git_utils_tests {
    ...
    fn should_get_current_branch_name() {
        ...
        assert_eq!(get_current_branch_name(&repo).unwrap(), "main");
    }

    #[test]
    fn should_convert_ref_name_from_branch_name() {
        assert_eq!(branch_name_to_ref_name("main"), "refs/heads/main");
        assert_eq!(branch_name_to_ref_name("test"), "refs/heads/test");
    }

    #[test]
    fn should_parse_branch_name() {
        assert_eq!(parse_branch_name("refs/heads/main").unwrap(), "main");
        assert_eq!(parse_branch_name("refs/heads/master").unwrap(), "master");
        assert_eq!(parse_branch_name("refs/heads/test").unwrap(), "test");
    }

    #[test]
    fn should_parse_error_when_ref_name_is_incorrect() {
        assert!(matches!(
            parse_branch_name("not_ref_name").unwrap_err(),
            GitError::InvalidBranchRef,
        ));
        assert!(matches!(
            parse_branch_name("refs/is_head/hello").unwrap_err(),
            GitError::InvalidBranchRef,
        ));
    }
}
